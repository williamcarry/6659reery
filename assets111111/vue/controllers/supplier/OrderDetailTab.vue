<template>
  <div class="order-detail-tab-container">
    ...
    <div class="order-detail" v-if="currentOrder" v-loading="detailLoading" element-loading-text="正在加载订单详情...">
      ...
      <el-card class="detail-card">
        ...
        <div v-loading="detailLoading" element-loading-text="正在加载..." style="min-height: 100px;">
          <div style="margin-top: 10px; padding: 10px; background-color: #f5f7fa; border-radius: 4px; display: flex; justify-content: space-between; align-items: center;">
            ...
            <!-- 一键全部发货按钮 -->
            <el-button 
              v-if="unshippedCount > 0"
              type="primary" 
              size="default"
              @click="handleShipAll"
              :loading="shipAllLoading"
            >
              <template #icon>
                <el-icon><Box /></el-icon>
              </template>
              一键全部发货 ({{ unshippedCount }}件)
            </el-button>
          </div>
        </div>
      </el-card>
    ...
    <el-dialog 
      ...
      :title="currentShippingItem ? '单个订单项发货' : '一键全部发货'"
      width="500px"
    ...
    >
      <el-form :model="shippingForm" label-width="100px">
        <el-form-item label="物流公司" required>
          <el-select 
            v-model="shippingForm.logistics_company_id" 
            placeholder="请选择物流公司"
            clearable
            filterable
            style="width: 100%;"
          >
            <el-option
              ...
              :key="company.id"
              :label="getLogisticsCompanyLabel(company)"
              :value="company.id"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="物流单号" required>
          <el-input 
            v-model="shippingForm.logistics_no" 
            placeholder="请输入物流单号"
            clearable
          />
        </el-form-item>
        <el-form-item v-if="currentShippingItem">
          <div style="font-size: 12px; color: #909399;">
            <div>商品：{{ currentShippingItem.productTitle }}</div>
            <div>SKU：{{ currentShippingItem.productSku }}</div>
            <div>数量：{{ currentShippingItem.quantity }}</div>
          </div>
        </el-form-item>
        <el-form-item v-else>
          <div style="font-size: 12px; color: #909399;">
            <div>将为 {{ unshippedCount }} 个未发货的订单项使用相同的物流信息</div>
          </div>
        </el-form-item>
      </el-form>
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="cancelShippingDialog">取消</el-button>
          <el-button 
            type="primary" 
            @click="submitShippingForm"
            :loading="shippingFormLoading"
          >
            确认发货
          </el-button>
        </span>
...
import { Document, Money, Location, Goods, EditPen, QuestionFilled, Box, Refresh, InfoFilled, View, Clock } from '@element-plus/icons-vue'
...
const shipAllLoading = ref(false)
const shippingDialogVisible = ref(false)
const shippingForm = ref({
  logistics_company_id: '',
  logistics_no: ''
})
const shippingFormLoading = ref(false)
const currentShippingItem = ref(null)
...
const initData = async () => {
  ...
  try {
    // 加载订单详情
    const detailResponse = await fetch(`/supplier${props.supplierLoginPath}/order/${props.orderId}`, {
      method: 'GET',
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      }
    ...
    // 加载订单明细
    const itemsResponse = await fetch(`/supplier${props.supplierLoginPath}/order/${props.orderId}/items`, {
      method: 'GET',
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      }
    ...
    if (itemsResult.success) {
      // 读取货币符号
      if (itemsResult.currencySymbol) {
        currencySymbol.value = itemsResult.currencySymbol
      }
...
// 计算未发货数量
const calculateUnshippedCount = () => {
  if (!orderItems.value || orderItems.value.length === 0) {
    unshippedCount.value = 0
    return
  }
  
  // 过滤掉物流信息行和退款原因行，只计算真实的商品行
  unshippedCount.value = orderItems.value.filter(item => 
    !item.isLogisticsRow && !item.isRefundReasonRow && item.orderStatus === 'paid' && !item.shippedTime
  ).length
}
...
// 获取物流公司显示名称（根据语言）
const getLogisticsCompanyLabel = (company) => {
  // TODO: 根据用户语言偏好返回不同的名称
  // 当前默认显示: 中文名 (English Name)
...
const handleShipAll = async () => {
  ...
  // 检查是否有自提订单项
  const unshippedItems = orderItems.value.filter(item => 
    item.orderStatus === 'paid' && !item.shippedTime
  )
  
  const hasSelfPickup = unshippedItems.some(item => item.shippingMethod === 'SELF_PICKUP')
  const selfPickupCount = unshippedItems.filter(item => item.shippingMethod === 'SELF_PICKUP').length
  const standardShippingCount = unshippedItems.length - selfPickupCount
  
  // 如果有自提订单，显示确认提示
  if (hasSelfPickup) {
    try {
      let confirmMessage = ''
      if (selfPickupCount === unshippedItems.length) {
        // 全部都是自提
        confirmMessage = `本次将发货 ${selfPickupCount} 个订单项，全部为自提订单（无运费）。\n\n确认发货吗？`
      } else {
        // 混合模式
        confirmMessage = `本次将发货 ${unshippedItems.length} 个订单项：

- 标准物流：${standardShippingCount} 个
- 自提订单（无运费）：${selfPickupCount} 个
...
确认发货吗？`
  ...
  try {
    ...
    // 打开发货弹窗
    currentShippingItem.value = null // null表示批量发货
    shippingForm.value = {
      logistics_company_id: '',
      logistics_no: ''
    }
...
const handleShipItem = async (item) => {
  if (!item || !item.id) {
    ElMessage.error('订单项ID不存在')
    return
  }
  
  // 检查是否为自提订单，如果是，先显示确认提示
  if (item.shippingMethod === 'SELF_PICKUP') {
    try {
      await ElMessageBox.confirm(
        `该订单项为自提订单，无运费。
...
SKU：${item.productSku}
数量：${item.quantity}
运费：${currencySymbol.value}0.00（自提）
...
确认发货吗？`,
  ...
  // 打开发货弹窗
  currentShippingItem.value = item
  shippingForm.value = {
    logistics_company_id: '',
    logistics_no: ''
  }
...
const submitShippingForm = async () => {
  // 验证表单
  if (!shippingForm.value.logistics_company_id || !shippingForm.value.logistics_no) {
    ElMessage.error('请填写完整的物流信息')
    return
  }
  
  shippingFormLoading.value = true
  
  try {
    let url = ''
    let method = 'POST'
    let body = {
      logistics_company_id: shippingForm.value.logistics_company_id,
      logistics_no: shippingForm.value.logistics_no
    }
    
    // 判断是单个发货还是批量发货
    if (currentShippingItem.value) {
      // 单个订单项发货
      url = `/supplier${props.supplierLoginPath}/api/order-shipping/ship-item/${currentShippingItem.value.id}`
    } else {
      // 批量发货
      url = `/supplier${props.supplierLoginPath}/api/order-shipping/ship-all/${currentOrder.value.orderNo}`
    }
    ...
    if (result.success) {
      ElMessage.success(result.message || '发货成功')
    ...
    shippingFormLoading.value = false
  }
}

// 取消发货弹窗
const cancelShippingDialog = () => {
  shippingDialogVisible.value = false
  shippingForm.value = {
    logistics_company_id: '',
    logistics_no: ''
  }
  currentShippingItem.value = null
}
...
const handleRefund = async (item) => {
  try {
    ...
    const action = await ElMessageBox.confirm(
      `订单项：${item.productTitle}\n退款金额：${currencySymbol.value}${item.refundAmount || item.subtotalAmount}\n\n请选择操作：`,
      '处理退款申请',
      {
        distinguishCancelAndClose: true,
    ...
    // 用户点击了确认按钮（同意退款）
    if (action === 'confirm') {
      await approveAndCompleteRefund(item)
    }
  } catch (action) {
    // 用户点击了取消按钮（拒绝退款）
    if (action === 'cancel') {
      await rejectRefund(item)
    }
  }
}

// 同意并完成退款（后端自动完成）
const approveAndCompleteRefund = async (item) => {
  try {
    ...
    // 调用同意退款接口，后端会自动完成退款
    const response = await fetch(`/supplier${props.supplierLoginPath}/api/order-shipping/approve-refund/${item.id}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest'
      ...
      body: JSON.stringify({
        refundQuantity: null // null表示全额退款
      })
    })
    ...
    if (result.success) {
      ElMessage.success(result.message || '退款已完成，已退款给买家并解冻供应商余额')
      // 重新加载订单详情
      await initData()
    } else {
      ElMessage.error(result.message || '同意退款失败')
    }
  } catch (error) {
...
const rejectRefund = async (item) => {
  try {
    ...
    const response = await fetch(`/supplier${props.supplierLoginPath}/api/order-shipping/reject-refund/${item.id}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest'
    ...
    if (error !== 'cancel') {
      ElMessage.error('拒绝退款失败: ' + error.message)
    }
  }
}

// 确认收到退货并完成退款
const handleConfirmRefundReceived = async (item) => {
  try {
    // 显示确认对话框
    await ElMessageBox.confirm(
      `订单项：${item.productTitle}
退款金额：${currencySymbol.value}${item.refundAmount || item.subtotalAmount}
退货物流：${item.refundLogisticsCompany?.name_zh || '-'}
物流单号：${item.refundLogisticsNo || '-'}

确认已收到买家退货？确认后将自动完成退款。`,
      '确认收货并完成退款',
      {
        confirmButtonText: '确认收货并退款',
        cancelButtonText: '取消',
        type: 'warning'
      }
    )
    ...
    // 调用确认收货接口
    const response = await fetch(`/supplier${props.supplierLoginPath}/api/order-shipping/confirm-refund-received/${item.id}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest'
    ...
    if (result.success) {
      ElMessage.success(result.message || '已确认收货并完成退款')
      // 重新加载订单详情
      await initData()
    } else {
...
// 判断订单项是否可以取消（待支付或已支付未发货）
const canCancelItem = (item) => {
  if (!item) return false
  // 待支付或已支付未发货状态可以取消
  return (item.orderStatus === 'pending_payment' || 
          (item.orderStatus === 'paid' && !item.shippedTime))
}

// 取消订单项（已支付未发货会自动退款并恢复库存）
const handleCancelItem = async (item) => {
  try {
    // 询问取消原因
    const { value: reason } = await ElMessageBox.prompt(
      item.orderStatus === 'paid' 
        ? `取消订单将自动退款给买家\n退款金额：${currencySymbol.value}${item.subtotalAmount}\n\n请输入取消原因：`
        : '请输入取消订单的原因',
      '取消订单项',
      {
    ...
    const response = await fetch(`/supplier${props.supplierLoginPath}/api/order-shipping/cancel-item/${item.id}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest'
    ...
    if (result.success) {
      ElMessage.success(result.message || '订单项已取消')
      // 重新加载订单详情
      await initData()
    } else {
