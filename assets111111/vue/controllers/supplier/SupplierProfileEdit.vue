<template>
  <div class="supplier-profile-container" v-loading="loading" element-loading-text="数据加载中..." element-loading-background="rgba(255, 255, 255, 0.9)">
    <!-- 页面头部 -->
    ...
    <el-card class="profile-card" shadow="hover">
      ...
      >
            ...
            <el-col :span="12">
              <el-form-item label="公司名称">
                <el-input 
                  ...
                  clearable
                />
                <div class="field-note">此项为审核必填资料，仅在"待重新提交"状态下可修改</div>
              </el-form-item>
            </el-col>
            <el-col :span="12">
        ...
        <el-row :gutter="30">
          <el-col :span="12">
            <el-form-item label="审核状态">
              <el-input :value="getAuditStatusText(formData.auditStatus)" disabled />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-form-item class="form-actions">
          ...
          <div v-if="formData.auditStatus === 'rejected'" class="reject-notice">
            <el-icon><InfoFilled /></el-icon>
            审核已拒绝，不可修改和提交。拒绝原因：{{ formData.auditRemark || '无' }}
          </div>
        </el-form-item>
      </el-form>
    </el-card>
...
// 计算属性：判断字段是否应该禁用（审核状态为approved、pending或rejected时禁用）
const shouldDisableFields = computed(() => {
  return formData.auditStatus === 'approved' || formData.auditStatus === 'pending' || formData.auditStatus === 'rejected';
});

// 计算属性：判断是否可以提交保存（rejected状态不能提交）
const canSubmit = computed(() => {
  return formData.auditStatus !== 'rejected';
});
