<template>
  <div class="order-container">
      ...
      <div class="order-detail" v-if="currentOrder" v-loading="detailLoading" element-loading-text="正在加载订单详情...">
        ...
        <el-card class="detail-card">
          ...
          <el-descriptions :column="3" border>
            ...
            <el-descriptions-item label="订单状态" label-class-name="desc-label">
        ...
        <el-card class="detail-card">
          ...
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="物流状态" label-class-name="desc-label">
            ...
            <el-table-column label="操作" width="200" align="center" fixed="right">
              <template #default="scope">
                <div class="button-group">
                  <!-- 待发货状态：显示发货按钮 -->
                  <el-button 
                    v-if="scope.row.orderStatus === 'paid' && !scope.row.shippedTime"
                    type="primary" 
                    size="small" 
                    @click="handleShipItem(scope.row)"
                  >
                    发货
                  </el-button>
                  ...
                  <!-- 已发货状态：显示查看物流按钮 -->
                  <el-button 
                    v-if="scope.row.shippedTime && scope.row.orderStatus === 'shipped'"
                    type="success" 
                    size="small" 
                  ...
                  <!-- 退款中状态：显示处理退款按钮 -->
                  <el-button 
                    v-if="scope.row.refundStatus === 'applying'"
                    type="warning" 
                    size="small" 
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
    >
      <el-form :model="shipForm" label-width="100px" v-loading="shipFormLoading">
        <el-form-item label="物流公司" required>
          ...
          >
            <el-option
              ...
              :key="company.id"
              :label="`${company.name_zh} / ${company.name_en}`"
              :value="company.id"
            >
              <div style="display: flex; flex-direction: column;">
                <span style="font-weight: 500;">{{ company.name_zh }}</span>
                <span style="font-size: 12px; color: #909399;">{{ company.name_en }}</span>
              </div>
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="订单项ID">
          <span style="color: #909399;">{{ currentShipItem?.id || '-' }}</span>
        </el-form-item>
        <el-form-item label="商品名称">
          <span>{{ currentShipItem?.productTitle || '-' }}</span>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="shipDialogVisible = false" :disabled="shipFormLoading">取消</el-button>
        <el-button type="primary" @click="confirmShip" :loading="shipFormLoading">确认发货</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :close-on-click-modal="false"
    >
      <div v-if="currentItemDetail" class="item-detail" v-loading="itemDetailLoading" element-loading-text="正在加载订单项详情...">
        <!-- 基本信息 -->
        <el-card class="detail-card">
          <template #header>
            <div class="detail-card-header">
          ...
          <el-descriptions :column="2" border>
            <el-descriptions-item label="订单项ID">
              {{ currentItemDetail.id }}
            </el-descriptions-item>
            <el-descriptions-item label="所属订单">
              {{ currentItemDetail.order.orderNo }}
            </el-descriptions-item>
            <el-descriptions-item label="买家">
              {{ currentItemDetail.order.customerUsername }}
            </el-descriptions-item>
            <el-descriptions-item label="下单时间">
              {{ currentItemDetail.order.createdAt }}
            </el-descriptions-item>
          </el-descriptions>
        </el-card>
        ...
        <el-card class="detail-card">
          ...
          <el-descriptions :column="2" border>
            <el-descriptions-item label="商品SKU">
              {{ currentItemDetail.product.sku }}
            </el-descriptions-item>
            <el-descriptions-item label="商品名称">
              {{ currentItemDetail.product.title }}
            </el-descriptions-item>
            <el-descriptions-item label="英文名称" :span="2">
              {{ currentItemDetail.product.titleEn || '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="发货区域">
              <el-tag type="primary" size="small">{{ currentItemDetail.shippingRegion }}</el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="供应商">
              {{ currentItemDetail.supplierName }}
            </el-descriptions-item>
            <el-descriptions-item label="商品规格" :span="2">
              <div v-if="currentItemDetail.productSpecs">
                <el-tag v-for="(value, key) in currentItemDetail.productSpecs" :key="key" type="info" size="small" style="margin-right: 5px;">
                  {{ key }}: {{ value }}
                </el-tag>
              </div>
              <span v-else>-</span>
        ...
        <el-card class="detail-card">
          ...
          <el-descriptions :column="3" border>
            <el-descriptions-item label="商品原价">
              <span style="color: #909399; text-decoration: line-through;">{{ currencySymbol }}{{ currentItemDetail.originalUnitPrice }}</span>
            </el-descriptions-item>
            <el-descriptions-item label="商品售价">
              <span style="color: #409eff; font-weight: bold;">{{ currencySymbol }}{{ currentItemDetail.unitPrice }}</span>
            </el-descriptions-item>
            <el-descriptions-item label="实付价格">
              <span style="color: #67c23a; font-weight: bold;">{{ currencySymbol }}{{ currentItemDetail.actualUnitPrice }}</span>
            </el-descriptions-item>
            <el-descriptions-item label="购买数量">
              {{ currentItemDetail.quantity }}
            </el-descriptions-item>
            <el-descriptions-item label="商品运费">
              {{ currencySymbol }}{{ currentItemDetail.shippingFee }}
            </el-descriptions-item>
            <el-descriptions-item label="支付总价">
              <span style="color: #f56c6c; font-weight: bold; font-size: 16px;">{{ currencySymbol }}{{ currentItemDetail.subtotalAmount }}</span>
            </el-descriptions-item>
            <el-descriptions-item label="佣金比例">
              <el-tag :type="currentItemDetail.commissionRate === '0.0000' ? 'success' : 'warning'" size="small">
                {{ formatCommissionRate(currentItemDetail.commissionRate) }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="平台收入">
              <span style="color: #e6a23c;">{{ currencySymbol }}{{ currentItemDetail.commissionAmount }}</span>
            </el-descriptions-item>
            <el-descriptions-item label="供应商收入">
              <span style="color: #67c23a; font-weight: bold; font-size: 16px;">{{ currencySymbol }}{{ currentItemDetail.supplierIncome }}</span>
            </el-descriptions-item>
          </el-descriptions>
        </el-card>

        <!-- 折扣信息 -->
        <el-card class="detail-card" v-if="currentItemDetail.discountDetails">
          <template #header>
            <div class="detail-card-header">
              <el-icon><Money /></el-icon>
              <span>折扣信息</span>
          ...
          <div style="display: flex; gap: 15px; flex-wrap: wrap; padding: 10px;">
            <el-tag v-if="parseFloat(currentItemDetail.discountDetails.product_discount_amount || 0) > 0" type="info">
              商品折扣：{{ currencySymbol }}{{ currentItemDetail.discountDetails.product_discount_amount }}
            </el-tag>
            <el-tag v-if="currentItemDetail.discountDetails.member_discount_rate" type="success">
              会员折扣：{{ formatDiscountRate(currentItemDetail.discountDetails.member_discount_rate) }} / {{ currencySymbol }}{{ currentItemDetail.discountDetails.member_discount_amount }}
            </el-tag>
            <el-tag v-if="parseFloat(currentItemDetail.discountDetails.full_reduction_amount || 0) > 0" type="warning">
              满减优惠：{{ currencySymbol }}{{ currentItemDetail.discountDetails.full_reduction_amount }}
            </el-tag>
            <el-tag type="danger">
              总折扣：{{ currencySymbol }}{{ currentItemDetail.discountDetails.total_discount_amount || '0.00' }}
            </el-tag>
          </div>
        </el-card>
        ...
        <el-card class="detail-card">
          ...
          <el-descriptions :column="2" border>
            <el-descriptions-item label="订单项状态">
              <el-tag :type="getOrderStatusTagType(currentItemDetail.orderStatus)">
                {{ getOrderStatusText(currentItemDetail.orderStatus) }}
              </el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="退款状态">
              <el-tag v-if="currentItemDetail.refundStatus !== 'none'" :type="getRefundStatusTagType(currentItemDetail.refundStatus)">
                {{ getRefundStatusText(currentItemDetail.refundStatus) }}
              </el-tag>
              <span v-else style="color: #909399;">-</span>
            </el-descriptions-item>
            <el-descriptions-item label="退款数量" v-if="currentItemDetail.refundQuantity">
              {{ currentItemDetail.refundQuantity }}
            </el-descriptions-item>
            <el-descriptions-item label="退款金额" v-if="currentItemDetail.refundAmount">
              <span style="color: #f56c6c;">{{ currencySymbol }}{{ currentItemDetail.refundAmount }}</span>
            </el-descriptions-item>
            <el-descriptions-item label="退款原因" :span="2" v-if="currentItemDetail.refundReason">
              {{ currentItemDetail.refundReason }}
            </el-descriptions-item>
            <el-descriptions-item label="退款时间" :span="2" v-if="currentItemDetail.refundTime">
              {{ currentItemDetail.refundTime }}
            </el-descriptions-item>
          </el-descriptions>
        </el-card>
        ...
        <el-card class="detail-card">
          ...
          <el-descriptions :column="2" border>
            <el-descriptions-item label="物流公司">
              <div v-if="currentItemDetail.logisticsCompany">
                <div style="font-weight: 500;">{{ currentItemDetail.logisticsCompany.name_zh }}</div>
                <div style="font-size: 12px; color: #909399; margin-top: 2px;">{{ currentItemDetail.logisticsCompany.name_en }}</div>
              </div>
              <span v-else style="color: #909399;">-</span>
            </el-descriptions-item>
            <el-descriptions-item label="物流单号">
              <span v-if="currentItemDetail.logisticsNo" style="color: #409eff; font-weight: 500;">{{ currentItemDetail.logisticsNo }}</span>
              <span v-else style="color: #909399;">-</span>
            </el-descriptions-item>
            <el-descriptions-item label="发货时间" :span="2">
              <span v-if="currentItemDetail.shippedTime">{{ currentItemDetail.shippedTime }}</span>
        ...
        <el-card class="detail-card">
          ...
          <el-descriptions :column="2" border>
            <el-descriptions-item label="创建时间">
              {{ currentItemDetail.createdAt }}
            </el-descriptions-item>
            <el-descriptions-item label="更新时间">
              {{ currentItemDetail.updatedAt }}
            </el-descriptions-item>
          </el-descriptions>
        </el-card>
      </div>
...
const getSignedUrl = async (qiniuKey) => {
  ...
  try {
    // 调用后端接口获取签名 URL
    const response = await fetch(`/supplier${props.supplierLoginPath}/order/qiniu/sign-url`, {
      method: 'POST',
      headers: {
...
const loadOrderData = async (page = 1, limit = 20) => {
  ...
  try {
    ...
    const url = `/supplier${props.supplierLoginPath}/order/list?${params}`
...
const loadLogisticsCompanies = async () => {
  try {
    const response = await fetch(`/supplier${props.supplierLoginPath}/api/order-shipping/logistics-companies`, {
      method: 'GET',
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      }
    ...
    if (result.success) {
      ...
      ElMessage.error(result.message || '加载物流公司列表失败')
    }
  } catch (error) {
    ElMessage.error('加载物流公司列表失败: ' + error.message)
  }
}
...
const confirmShip = async () => {
  if (!shipForm.value.logisticsCompanyId) {
    ElMessage.warning('请选择物流公司')
    return
  }
  
  if (!shipForm.value.logisticsNo || shipForm.value.logisticsNo.trim() === '') {
  ...
  try {
    const response = await fetch(`/supplier${props.supplierLoginPath}/api/order-shipping/ship-item/${currentShipItem.value.id}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest'
      ...
      body: JSON.stringify({
        logistics_company_id: shipForm.value.logisticsCompanyId,
        logistics_no: shipForm.value.logisticsNo.trim()
      })
    })
    ...
    if (result.success) {
      ...
      if (currentOrder.value) {
        await viewOrderDetail(currentOrder.value)
      }
    } else {
...
// 查看物流信息
const handleViewLogistics = (item) => {
  if (item.logisticsCompany && item.logisticsNo) {
    const companyName = item.logisticsCompany.name_zh || item.logisticsCompany.name_en
    const message = `物流公司：${companyName}<br/>物流单号：${item.logisticsNo}<br/>发货时间：${item.shippedTime || '-'}`
...
const handleRefund = (item) => {
  ElMessage.info(`退款功能正在开发中，订单项 ID: ${item.id}`)
  // TODO: 实现退款处理对话框逻辑
}
...
const handleViewItemDetail = async (item) => {
  ...
  try {
    const response = await fetch(`/supplier${props.supplierLoginPath}/order/item/${item.id}`, {
      method: 'GET',
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      }
    ...
    if (result.success) {
      currentItemDetail.value = result.data
    } else {
      ElMessage.error(result.message || '加载订单项详情失败')
      itemDetailDialogVisible.value = false
    }
  } catch (error) {
    ElMessage.error('加载订单项详情失败: ' + error.message)
    itemDetailDialogVisible.value = false
  } finally {
...
.item-detail .detail-card {
  margin-bottom: 20px;
}

.item-detail .detail-card:last-child {
  margin-bottom: 0;
}
</style>
