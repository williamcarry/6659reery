  ...
  <div class="withdrawal-list">
        ...
        <el-row :gutter="20" align="middle">
          ...
          <el-col :span="5" :xs="24">
            <el-form-item label="用户名称">
              <el-input v-model="searchForm.supplierName" placeholder="请输入用户名称" />
            </el-form-item>
          </el-col>
          <el-col :span="6" :xs="24">
    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column label="提现用户" min-width="200">
          <template #default="scope">
            <!-- 用户类型标签 -->
            <el-tag 
              :type="scope.row.userType === 'supplier' ? 'success' : 'primary'" 
              size="small" 
              style="margin-right: 8px;"
            >
              {{ scope.row.userType === 'supplier' ? '供应商' : '会员' }}
            </el-tag>
            <br />
            <!-- 用户信息 -->
            <span style="color: #409eff;">{{ scope.row.username }}</span>
            <span>/</span>
            <span style="color: #67c23a;">{{ scope.row.displayName || '无' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="amount" label="提现金额" min-width="100">
          <template #default="scope">
            <span style="color: #f56c6c;">{{ currencySymbol }}{{ formatCurrency(scope.row.amount) }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="detailDialogVisible" title="提现申请详情" width="600px">
      <el-form :model="currentWithdrawal" label-width="100px">
        <el-form-item label="订单号：">
          <span>{{ currentWithdrawal.orderNo || '-' }}</span>
        </el-form-item>
        <el-form-item label="提现用户：">
          <!-- 用户类型标签 -->
          <el-tag 
            :type="currentWithdrawal.userType === 'supplier' ? 'success' : 'primary'" 
            size="small" 
            style="margin-right: 8px;"
          >
            {{ currentWithdrawal.userType === 'supplier' ? '供应商' : '会员' }}
          </el-tag>
          <br />
          <span style="color: #409eff;">{{ currentWithdrawal.username || '-' }}</span>
          <span>/</span>
          <span style="color: #67c23a;">{{ currentWithdrawal.displayName || '无' }}</span>
        </el-form-item>
        <el-form-item label="提现金额：">
          <span style="color: #f56c6c; font-weight: bold;">{{ currencySymbol }}{{ formatCurrency(currentWithdrawal.amount) }}</span>
        </el-form-item>
