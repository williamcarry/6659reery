  ...
  <div class="site-config-manager">
    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="configValue2" label="配置值二" min-width="200" show-overflow-tooltip>
          <template #default="scope">
            <div class="config-value-cell">
              {{ scope.row.configValue2 || '-' }}
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="configValue3" label="配置值三" min-width="200" show-overflow-tooltip>
          <template #default="scope">
            <div class="config-value-cell">
              {{ scope.row.configValue3 || '-' }}
            </div>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px">
      <el-form :model="configForm" label-width="100px" ref="configFormRef">
        <el-form-item label="配置键名" prop="configKey" :rules="[
          ...
          <div v-if="isEditMode && isSiteCurrencyConfig(configForm.configKey)" class="form-item-tip">
            site_currency 配置项的配置键名不可修改
          </div>
        </el-form-item>
        <el-form-item label="配置值" prop="configValue">
          <!-- 特殊处理 withdrawRefundUseOnlinePay 参数 -->
          <el-select v-if="configForm.configKey === 'withdrawRefundUseOnlinePay'" v-model="configForm.configValue" placeholder="请选择">
            <el-option label="在线" value="online" />
            <el-option label="余额" value="offline" />
          </el-select>
          <!-- 特殊处理图片配置项 -->
          <div v-else-if="isImageConfig(configForm.configKey)" class="image-upload-section">
            <file-upload 
          ...
          <el-input v-else v-model="configForm.configValue" type="textarea" :rows="4" :disabled="isSiteCurrencyConfig(configForm.configKey)" />
          <div v-if="isSiteCurrencyConfig(configForm.configKey)" class="form-item-tip">
            site_currency 配置项的配置值不可修改
          </div>
        </el-form-item>
        <el-form-item label="配置值二" prop="configValue2">
          <el-input v-model="configForm.configValue2" type="textarea" :rows="4" :disabled="isSiteCurrencyConfig(configForm.configKey) || configForm.configKey === 'withdrawRefundUseOnlinePay'" />
          <div v-if="isSiteCurrencyConfig(configForm.configKey)" class="form-item-tip">
            site_currency 配置项的配置值二不可修改
          </div>
          <div v-else-if="configForm.configKey === 'withdrawRefundUseOnlinePay'" class="form-item-tip">
            withdrawRefundUseOnlinePay 配置项的配置值二不可修改
          </div>
        </el-form-item>
        <el-form-item label="配置值三" prop="configValue3">
          <el-input v-model="configForm.configValue3" type="textarea" :rows="4" :disabled="isSiteCurrencyConfig(configForm.configKey) || configForm.configKey === 'withdrawRefundUseOnlinePay'" />
          <div v-if="isSiteCurrencyConfig(configForm.configKey)" class="form-item-tip">
            site_currency 配置项的配置值三不可修改
          </div>
          <div v-else-if="configForm.configKey === 'withdrawRefundUseOnlinePay'" class="form-item-tip">
            withdrawRefundUseOnlinePay 配置项的配置值三不可修改
          </div>
        </el-form-item>
...
// 判断是否为 site_currency 配置项
const isSiteCurrencyConfig = (configKey) => {
  return configKey && configKey.startsWith('site_currency')
}
...
const editConfig = (config) => {
  ...
  configForm.value = { 
    ...
    configValue3: config.configValue3,
...
const deleteConfig = (config) => {
  ElMessage.warning('网站参数不允许删除，如需修改请使用编辑功能')
  return
}
...
const saveConfig = async () => {
  ...
  try {
    ...
    const formData = {
      ...
      configValue2: configForm.value.configValue2,
      configValue3: configForm.value.configValue3,
