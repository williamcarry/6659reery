...
export default {
  ...
  methods: {
    async loadItemData(page = 1, categoryId = 0, subcategoryId = 0) {
      this.loading = true
      this.error = null
      ...
      try {
        ...
        // 获取三级分类数据
        const itemResponse = await fetch(`/admin${loginPath}/menu/item/list/data?${params}`, {
          method: 'GET',
          headers: {
        ...
        // 获取二级分类数据（用于选择框）
        const subcategoryParams = new URLSearchParams({ page: 1 });
        if (categoryId > 0) {
          subcategoryParams.append('categoryId', categoryId);
        }
        
        const subcategoryResponse = await fetch(`/admin${loginPath}/menu/subcategory/list/data?${subcategoryParams}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          }
        ...
        if (itemResult.success && subcategoryResult.success && categoryResult.success) {
          ...
          this.error = itemResult.message || subcategoryResult.message || categoryResult.message || '加载数据失败'
        }
      } catch (err) {
    ...
    handlePageChanged(event) {
      // 当页面更改时，加载指定页面的数据
      const categoryId = event.detail.categoryId || 0;
      const subcategoryId = event.detail.subcategoryId || 0;
      this.loadItemData(event.detail.page, categoryId, subcategoryId)
    },
  ...
  mounted() {
    ...
    window.addEventListener('menu-item-search', this.handleSearch)
  },
  beforeUnmount() {
    // 清理事件监听器
    ...
    window.removeEventListener('menu-item-search', this.handleSearch)
  }
}
</script>
