    ...
    const loadSupplierDetail = async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          // 对于图片字段，优先使用带签名的URL（如businessLicenseImageUrl），如果不存在则使用原始key
          const supplierCopy = {
            ...supplierData,
            // 公司证件图片 - 优先使用带签名的URL
            businessLicenseImage: supplierData.businessLicenseImageUrl || supplierData.businessLicenseImage || '',
            legalPersonIdFront: supplierData.legalPersonIdFrontUrl || supplierData.legalPersonIdFront || '',
            legalPersonIdBack: supplierData.legalPersonIdBackUrl || supplierData.legalPersonIdBack || '',
            // 个人证件图片 - 优先使用带签名的URL
            individualIdFront: supplierData.individualIdFrontUrl || supplierData.individualIdFront || '',
            individualIdBack: supplierData.individualIdBackUrl || supplierData.individualIdBack || ''
          }
