<template>
  <div class="member-statement-container">
    ...
    <!-- 会员信息卡片 -->
    <el-card class="member-info-card" v-loading="loading">
      <template #header>
      ...
      <el-descriptions :column="3" border v-if="memberInfo">
        ...
        <el-descriptions-item label="联系方式">
          <div style="display: flex; flex-direction: column; gap: 4px;">
            <div><span style="color: #909399; font-size: 12px;">昵称：</span>{{ memberInfo.nickname || '-' }}</div>
            <div><span style="color: #909399; font-size: 12px;">姓名：</span>{{ memberInfo.realName || '-' }}</div>
            <div><span style="color: #909399; font-size: 12px;">手机：</span>{{ memberInfo.mobile || '-' }}</div>
            <div><span style="color: #909399; font-size: 12px;">邮箱：</span>{{ memberInfo.email || '-' }}</div>
          </div>
        </el-descriptions-item>
        <el-descriptions-item label="账户余额">
          <span class="balance-value">{{ currencySymbol }}{{ formatCurrency(memberInfo.balance) }}</span>
        </el-descriptions-item>
        <el-descriptions-item label="冻结余额">
          <span class="frozen-value">{{ currencySymbol }}{{ formatCurrency(memberInfo.frozenBalance) }}</span>
        </el-descriptions-item>
        <el-descriptions-item label="可用余额">
          <span class="available-value">{{ currencySymbol }}{{ formatCurrency(getAvailableBalance()) }}</span>
        </el-descriptions-item>
        <el-descriptions-item label="计算账户余额">
          <span :class="getCalculatedBalanceClass('total')">{{ currencySymbol }}{{ formatCurrency(calculatedBalance.total) }}</span>
        </el-descriptions-item>
    ...
    <!-- 余额校验按钮 -->
    <div class="verify-actions" v-if="memberInfo">
      <el-button 
        type="primary" 
...
const getAvailableBalance = () => {
  if (!memberInfo.value) return '0.00'
  const balance = parseFloat(memberInfo.value.balance || 0)
  const frozen = parseFloat(memberInfo.value.frozenBalance || 0)
...
const loadMemberInfo = async () => {
  ...
  loading.value = true
  try {
    const response = await fetch(`/admin${props.adminLoginPath}/member-statement/${props.memberId}/info`, {
      method: 'GET',
      headers: {
    ...
    if (result.success) {
      ...
      ElMessage.error(result.message || '加载会员信息失败')
    }
  } catch (error) {
    ElMessage.error('加载会员信息失败: ' + error.message)
  } finally {
    loading.value = false
  }
}
...
const loadBalanceHistories = async (page = 1, limit = 20) => {
  ...
  try {
    ...
    const response = await fetch(`/admin${props.adminLoginPath}/member-statement/${props.memberId}/data?${params}`, {
      method: 'GET',
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      }
...
const calculateBalance = async () => {
  ...
  try {
    const response = await fetch(`/admin${props.adminLoginPath}/member-statement/${props.memberId}/calculate-balance`, {
      method: 'POST',
      headers: {
    ...
    if (result.success) {
      ...
      // 更新计算结果
      calculatedBalance.value = {
        total: data.calculatedTotal || '0.00',
        frozen: data.calculatedFrozen,
        available: data.calculatedAvailable,
        calculated: true,
        totalMatch: data.totalMatch !== undefined ? data.totalMatch : true,
        frozenMatch: data.frozenMatch,
        availableMatch: data.availableMatch
      }

      // 生成提示信息（一行一行）
      if (!data.frozenMatch || !data.availableMatch || (data.totalMatch !== undefined && !data.totalMatch)) {
        verifyResults.value.push({ type: 'error', message: '余额校验不匹配！' })
        verifyResults.value.push({ type: 'divider', message: '' })
        
        if (data.totalMatch !== undefined && !data.totalMatch) {
          const diff = (parseFloat(data.actualTotal || data.actualBalance || 0) - parseFloat(data.calculatedTotal || 0)).toFixed(2)
          verifyResults.value.push({ 
            type: 'mismatch', 
            message: `账户余额差异: ¥${diff}`,
            actual: `实际值: ¥${data.actualTotal || data.actualBalance || '0.00'}`,
            calculated: `计算值: ¥${data.calculatedTotal || '0.00'}`,
            actualClass: 'balance-color',
            calculatedClass: 'balance-color'
          })
        }
...
const getCalculatedBalanceClass = (type) => {
  ...
  if (type === 'total') {
    return calculatedBalance.value.totalMatch ? 'calculated-value match' : 'calculated-value mismatch'
  } else if (type === 'frozen') {
