...
export default {
  ...
  methods: {
    async loadSubcategoryData(page = 1, categoryId = 0) {
      this.loading = true
      this.error = null
      ...
      try {
        ...
        // 获取二级分类数据
        const subcategoryResponse = await fetch(`/admin${loginPath}/menu/subcategory/list/data?${params}`, {
          method: 'GET',
          headers: {
    ...
    handlePageChanged(event) {
      ...
      this.loadSubcategoryData(event.detail.page, categoryId)
    },
  ...
  mounted() {
    ...
    window.addEventListener('menu-subcategory-search', this.handleSearch)
  },
  beforeUnmount() {
    // 清理事件监听器
    ...
    window.removeEventListener('menu-subcategory-search', this.handleSearch)
  }
}
</script>
