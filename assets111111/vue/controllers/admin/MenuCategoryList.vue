<template>
  <div class="menu-category-list-container">
    ...
    <div class="page-header">
      <h2 class="page-title">
        <el-icon class="title-icon"><LayoutIcon /></el-icon>
        一级分类管理
      </h2>
      <div class="header-actions">
        <el-button type="primary" @click="showCreateDialog" :icon="PlusIcon">
          添加一级分类
        </el-button>
        <el-button 
          type="success" 
          @click="pushToCache" 
          :loading="pushCacheLoading"
        >
          <el-icon v-if="!pushCacheLoading" class="mr-1"><UploadIcon /></el-icon>
          {{ pushCacheLoading ? '正在压入缓存...' : '压入缓存' }}
        </el-button>
      </div>
    </div>
    ...
    <el-card class="table-card">
      <template #header>
        <div class="table-header">
          <div class="table-header-title">
            <el-icon class="table-icon"><ListIcon /></el-icon>
            一级分类列表
          </div>
      ...
      >
        ...
        <el-table-column prop="image" label="分类图片" min-width="120">
          <template #default="scope">
            <div v-if="scope.row.image" class="image-display">
              <el-image 
                :src="scope.row.image" 
      ...
      <el-form :model="currentCategory" label-width="100px">
        ...
        <el-form-item label="分类图片" required>
          <el-upload
            ...
            accept="image/*"
          >
            <img v-if="currentCategory.imagePreview" :src="currentCategory.imagePreview" class="uploaded-image" />
            <el-icon v-else class="uploader-icon"><PlusIcon /></el-icon>
          </el-upload>
...
import { Menu, Plus, List, Upload } from '@element-plus/icons-vue'

// 使用 markRaw 避免图标组件被响应式化
const LayoutIcon = markRaw(Menu)  // 使用 Menu 代替 Layout
...
const pushCacheLoading = ref(false)
...
// 上传相关
const uploadUrl = computed(() => `/admin${window.adminLoginPath}/menu/category/upload-image`)
const uploadHeaders = {}
...
const handleImageSuccess = (response, file) => {
  if (response.success) {
    currentCategory.image = response.data.filename
    currentCategory.imagePreview = response.data.url
...
const showEditDialog = (category) => {
  ...
  let imageFilename = ''
  if (category.image) {
    imageFilename = category.image.split('/').pop()
  }
  Object.assign(currentCategory, { 
    ...category,
    image: imageFilename,
...
const pushToCache = async () => {
  try {
    pushCacheLoading.value = true
    
    // 使用从prop传入的adminLoginPath
    const loginPath = props.adminLoginPath || window.adminLoginPath || ''
    const response = await fetch(`/admin${loginPath}/menu/category/push-cache`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      }
    ...
    if (result.success) {
      ...
      ElMessage({
        type: 'error',
        message: result.message || '压入缓存失败',
      })
    }
  } catch (error) {
    ElMessage({
      type: 'error',
      message: '压入缓存失败: ' + error.message,
    })
  } finally {
    pushCacheLoading.value = false
  }
}
...
.category-image-uploader :deep(.el-upload) {
...
.category-image-uploader :deep(.el-upload:hover) {
  border-color: #409eff;
}
